...
function SelectTrigger({ className, children, size = 'default', withoutIcon, icon, ...props }: SelectTriggerProps) {
  return (
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          'data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-(--radix-select-content-transform-origin)',
          position === 'popper' &&
...
function SelectItem({ className, children, plain, ...props }: SelectItemProps) {
  return (
    <SelectPrimitive.Item
      data-slot='item'
      className={cn(
        'focus:bg-ac/10 focus:text-ac relative flex w-full cursor-default items-start gap-1 px-2 py-1.5 text-sm outline-hidden select-none',
